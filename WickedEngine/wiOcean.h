...
namespace wi
{
	...
	public:
		...
		void UpdateDisplacementMap(wi::CommandList cmd) const;
		void RenderForOcclusionTest(const wi::scene::CameraComponent& camera, wi::CommandList cmd) const;
		void RenderForCubemap(wi::CommandList cmd) const;
		void Render(const wi::scene::CameraComponent& camera, wi::CommandList cmd) const;

		void CopyDisplacementMapReadback(wi::CommandList cmd) const;

		const wi::Texture* getDisplacementMap() const;
		const wi::Texture* getGradientMap() const;
		...
		mutable int occlusionQueries[wi::GraphicsDevice::GetBufferCount()];
	...
	protected:
		wi::Texture displacementMap;		// (RGBA32F)
		wi::Texture gradientMap;			// (RGBA16F)

		wi::Texture displacementMap_readback[wi::GraphicsDevice::GetBufferCount()];		// (RGBA32F)
		...
		wi::GPUBuffer buffer_Float2_H0;

		// Angular frequency
		wi::GPUBuffer buffer_Float_Omega;

		// Height field H(t), choppy field Dx(t) and Dy(t) in frequency domain, updated each frame.
		wi::GPUBuffer buffer_Float2_Ht;

		// Height & choppy buffer in the space domain, corresponding to H(t), Dx(t) and Dy(t)
		wi::GPUBuffer buffer_Float_Dxyz;


		wi::GPUBuffer constantBuffer;
		mutable wi::GPUBuffer indexBuffer;
		mutable wi::GPUBuffer indexBuffer_occlusionTest;
		mutable wi::GPUBuffer indexBuffer_cubemap;
	};
}

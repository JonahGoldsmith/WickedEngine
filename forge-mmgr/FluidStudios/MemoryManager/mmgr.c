...
#if __has_include(<SDL3/SDL_assert.h>)
#include <SDL3/SDL_assert.h>
#define MMGR_HAS_SDL_ASSERT 1
#elif __has_include(<SDL2/SDL_assert.h>)
#include <SDL2/SDL_assert.h>
#define MMGR_HAS_SDL_ASSERT 1
#elif __has_include(<SDL_assert.h>)
#include <SDL_assert.h>
#define MMGR_HAS_SDL_ASSERT 1
#else
#define MMGR_HAS_SDL_ASSERT 0
#endif
...
#if !defined(UNREF_PARAM)
#define UNREF_PARAM(x) ((void)(x))
#endif

#if !defined(ASSERT)
#if MMGR_HAS_SDL_ASSERT
...
#define ASSERT(condition)                                                                                    \
    ...
    {                                                                                                        \
        if (!(condition))                                                                                    \
        {                                                                                                    \
            fprintf(stderr, "mmgr assertion failed: %s (%s:%d)\n", #condition, __FILE__, __LINE__);        \
            assert(condition);                                                                               \
        }                                                                                                    \
    } while (0)
#endif
...
static bool fsOpenStreamFromPath(int resourceDir, const char* fileName, int mode, FileStream* out)
{
    ...
    const char* fopenMode = (mode & FM_APPEND) ? "ab" : "wb";
    FILE* fp = fopen(fileName, fopenMode);
    out->handle = fp;
    return fp != NULL;
}
static size_t fsWriteToStream(FileStream* stream, const void* data, size_t bytes)
{
    if (stream == NULL || stream->handle == NULL || data == NULL || bytes == 0)
    {
        return 0;
    }
    return fwrite(data, 1, bytes, stream->handle);
}
static bool fsFlushStream(FileStream* stream)
{
    if (stream == NULL || stream->handle == NULL)
    {
        return false;
    }
    return fflush(stream->handle) == 0;
}
static bool fsCloseStream(FileStream* stream)
{
    if (stream == NULL || stream->handle == NULL)
    {
        return false;
    }
    const int rc = fclose(stream->handle);
    stream->handle = NULL;
    return rc == 0;
}
...
static void dumpLine(FileStream* fileToWrite, const char* format, ...)
{
    ...
    _OutputDebugString("\n");
    if (fileToWrite != NULL && fileToWrite->handle != NULL)
    {
...
static bool isKnownExternalRuntimeModule(const char* module_name)
{
    if (module_name == NULL)
    {
        return false;
    ...
    const unsigned int ignored_module_count = (unsigned int)(sizeof(ignored_modules) / sizeof(ignored_modules[0]));
    for (unsigned int i = 0; i < ignored_module_count; ++i)
    {
        if (containsSubstring(module_name, ignored_modules[i]))
        {
            return true;
        }
    }
...
static bool isIgnoredLeak(const sAllocUnit* ptr)
{
    if (ptr == NULL)
    {
        return true;
    ...
    if (containsSubstring(ptr->sourceFunc, "gVkAllocation") || containsSubstring(ptr->sourceFunc, "gVkReallocation"))
    {
        return true;
    }
#endif
    ...
    // Global new/delete interception can include long-lived platform runtime caches.
    // Keep MMGR strict for engine/sample leaks, but suppress known external-runtime allocations.
    const int start = ptr->backtrace_skip < ptr->backtrace_nptrs ? ptr->backtrace_skip : 0;
    const int end = ptr->backtrace_nptrs < (start + 16) ? ptr->backtrace_nptrs : (start + 16);
    for (int i = start; i < end; ++i)
    {
        Dl_info info;
        memset(&info, 0, sizeof(info));
        if (dladdr(ptr->backtrace_buffer[i], &info) != 0)
        {
            if (isKnownExternalRuntimeModule(info.dli_fname))
            {
                return true;
            }
        }
...
static void computeLeakSummary(unsigned int* actionable_leaks, unsigned int* ignored_leaks)
{
    unsigned int actionable = 0;
    ...
    if (actionable_leaks != NULL)
    {
        *actionable_leaks = actionable;
    }
    if (ignored_leaks != NULL)
    {
        *ignored_leaks = ignored;
    }
}

static void dumpAllocations(FileStream* fh, const bool include_ignored)
{
    ...
    for (unsigned int i = 0; i < hashSize; i++)
    {
        ...
        while (ptr)
        {
            if (!include_ignored && isIgnoredLeak(ptr))
            {
                ptr = ptr->next;
                continue;
            }
...
static void dumpLeakReport(void)
{
    ...
    unsigned int actionable_leaks = 0;
    unsigned int ignored_leaks = 0;
    computeLeakSummary(&actionable_leaks, &ignored_leaks);

    {
        ...
        if (actionable_leaks)
        {
            dumpLine(&fh, "%u actionable memory leak%s found (%u external/runtime allocation%s ignored):\n", actionable_leaks,
                     actionable_leaks == 1 ? "" : "s", ignored_leaks, ignored_leaks == 1 ? "" : "s");
        }
        else
        {
            if (ignored_leaks > 0)
            {
                dumpLine(&fh, "No actionable memory leaks found. Ignored %u external/runtime allocation%s.", ignored_leaks,
                         ignored_leaks == 1 ? "" : "s");
            }
            else
            {
            ...
            if (stats.totalAllocUnitCount == 0 && reservoirBuffer)
            {
    ...
    ASSERT(actionable_leaks == 0 && "Memory leaks found");
}
// ---------------------------------------------------------------------------------------------------------------------------------
...
bool initMemAlloc(const char* appName)
{
    ...
    ASSERT(allocMutex != NULL);
    ASSERT(logMutex != NULL);
    if (allocMutex == NULL || logMutex == NULL)
    {
        return false;
    }
...
void exitMemAlloc(void)
{
    ...
    // Keep mutexes alive after report generation because process-level static
    // destructors can still call global delete/free through MMGR overrides.
    // Final process teardown releases these OS resources.
}
                      ...
                      size_t reportedSize, void* reportedAddress)
{
...

    {
        ...
        if (au == NULL)
        {
            // External allocator / startup allocations can legitimately hit here.
            // Fall back to raw realloc instead of asserting while MMGR lock is held.
            void* fallback = realloc(reportedAddress, reportedSize);
            MUTEX_UNLOCK(allocMutex);
            return fallback;
        }
                     ...
                     const void* reportedAddress)
{
    ...
    if (reportedAddress)
    {
        ...
        {
            // External allocator / startup allocations can legitimately hit here.
            // Fall back to raw free instead of asserting while MMGR lock is held.
...
unsigned int mmgrCalcUnused(const sAllocUnit* allocUnit)
{
    ...
    for (unsigned int i = 0; i < allocUnit->reportedSize; ++i, ++ptr)
    {
        const uint8_t expected = (uint8_t)((unusedPattern >> ((i % sizeof(uint32_t)) * 8)) & 0xFF);
        if (*ptr == expected)
            ++count;
        else
            break;
